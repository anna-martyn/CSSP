...
alpha <- read.table(
  "../1_scripts/allASVs/LotusSC_allASVs_chao1.txt",
  sep = "\t",
  header = TRUE,
  row.names = 1,
  check.names = FALSE
...
alpha_filt <- read.table(
  "../1_scripts/matchedASVsonly/LotusSC_matchedASVsonly_chao1.txt",
  sep = "\t",
  header = TRUE,
  row.names = 1,
  check.names = FALSE
...
run_analysis_plot <- function(df, colors, title, output_pdf, output_rds) {
  ...
  saveRDS(
    combined,
    file = file.path("../../../3_final_figures", basename(output_rds))
  )
}
...
run_analysis_plot(
  ...
  colors = colors,
  title = "Chao1 index",
  output_pdf = "LotusSC_chao1_allASVs_rfd_combined.pdf",
  output_rds = "LotusSC_chao1_allASVs_rfd_combined.rds"
)
...
run_analysis_plot(
  ...
  colors = colors,
  title = "Chao1 index",
  output_pdf = "LotusSC_chao1_matchedASVsonly_rfd_combined.pdf",
  output_rds = "LotusSC_chao1_matchedASVsonly_rfd_combined.rds"
)
