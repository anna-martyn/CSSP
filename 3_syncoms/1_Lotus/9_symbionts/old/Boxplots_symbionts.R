...
# Set genotype order for later plots, as well as make mutant names italic.
colors_geno <- c(
  "WT"     = "#A9C289",
  "symrk"  = "#FEDA8B",
  "ccamk"  = "#FDB366",
...
p_box <- ggplot(df_plot, aes(x=Compartment, y=RA, fill=Genotype)) +
  ...
  geom_jitter(position = position_jitterdodge(jitter.width = 0, dodge.width = 0.8), size=3, alpha=0.3) +
  geom_text(aes(x=Compartment, y=y_pos, label=Letter, fill=Genotype), position=dodge, size=5, inherit.aes=FALSE) +
