...
# One ccamk rhizosphere sample has <1000 reads. We will remove it for rarefication.
...
write.csv(ASVtable_rarefied, file =  "HordeumSC_rfd_woinput_min1000reads.csv")
write.table(ASVtable_rarefied, 
            file = "HordeumSC_rfd_woinput_min1000reads.txt", 
            sep = "\t",
            row.names = FALSE,
            col.names = TRUE, 
            quote = FALSE)

write.csv(ASVtable_rarefied_filt, file =  "HordeumSC_rfd_woinput_min1000reads_nocontaminants.csv")
write.table(ASVtable_rarefied_filt, 
            file = "HordeumSC_rfd_woinput_min1000reads_nocontaminants.txt", 
            sep = "\t",
            row.names = FALSE,
            col.names = TRUE, 
            quote = FALSE)
