...
alpha <- read.table("../../1_Scripts/without_input/allASVs/HordeumSC_allASVs_chao1.txt",
                    sep="\t", header=TRUE, row.names=1, check.names=FALSE)
alpha_filt <- read.table("../../1_Scripts/without_input/matchedASVsonly/HordeumSC_matchedASVsonly_chao1.txt",
                         sep="\t", header=TRUE, row.names=1, check.names=FALSE)
design <- read.table("../../../1_data/without_input/HordeumSC_metadata.txt", sep="\t", header=TRUE, row.names=1, check.names=FALSE)
...
run_analysis_plot <- function(df, colors, title, output_pdf, output_rds){
  ...
  p1 <- plot_facet(df, plot_summary, anova_pvals, colors, y_limits=y_limits, y_breaks=y_breaks) +
    ggtitle("Hordeum") +
  ...
  saveRDS(p1, file = file.path("../../../../3_final_figures", basename(output_rds)))
}

# Run the function for both datasets (matched ASVs only, and all ASVs).
run_analysis_plot(index, colors, "Chao1 index",
                  "HordeumSC_chao1_allASVs_rfd.pdf",
                  "HordeumSC_chao1_allASVs_rfd.rds")

run_analysis_plot(index_filt, colors, "Chao1 index",
                  "HordeumSC_chao1_matchedASVsonly_rfd.pdf",
                  "HordeumSC_chao1_matchedASVsonly_rfd.rds")
