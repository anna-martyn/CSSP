...
asv_tables_subsets <- apply(
  opt, 1,
  function(x){
    current_compartment <- x[1]
    current_soil <- x[2]

    # Subsetting data for compartment-soil combination
    data_subset <- asv_table_with_metadata[
    ...
    col_asv_table <- setdiff(colnames(data_subset), c("SampleID", col_design))
    asv_table <- t(data_subset[,..col_asv_table])
...
combined_top_orders <- readRDS("../0_files/Orders_to_display.rds")
...
heatmap <- ggplot(
  ...
  scale_fill_manual(
    values = c("Depleted" = "darkblue", "Enriched" = "#902121", "NS" = "white"),
