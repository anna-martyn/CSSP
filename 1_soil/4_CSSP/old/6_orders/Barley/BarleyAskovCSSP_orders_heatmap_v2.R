...
# Load files.
design.file <- paste("BarleyCSSP_Askov_reseq_metadata-2.txt", sep="")
...
df.long <- reshape(df, varying = 1:191, direction = "long", idvar = 'ASV', timevar="sampleID", v.names=c("RA"), 
                   times=c(colnames(df[,1:191])), sep="")
...
df3 <- data.frame(sampleID = design$Sample_ID, Genotype=design$Genotype, Compartment = design$Compartment, Plant=design$Plant, Soil=design$Soil)
...
ggsave(paste("BarleyCSSP_heatmap_orders_above_equal1perc.png", sep=""), p1, width=14, height=13)
ggsave(paste("BarleyCSSP_heatmap_orders_above_equal1perc.pdf", sep=""), p1, width=14, height=13)
...
write.xlsx(final_df, file = "BarleyCSSP_rhizo_NPK_ANOVA.xlsx", row.names = FALSE)
...
write.xlsx(final_df, file = "BarleyCSSP_rhizo_PK_ANOVA.xlsx", row.names = FALSE)
...
write.xlsx(final_df, file = "BarleyCSSP_rhizo_UF_ANOVA.xlsx", row.names = FALSE)
...
write.xlsx(final_df, file = "BarleyCSSP_root_NPK_ANOVA.xlsx", row.names = FALSE)
...
write.xlsx(final_df, file = "BarleyCSSP_root_PK_ANOVA.xlsx", row.names = FALSE)
...
write.xlsx(final_df, file = "BarleyCSSP_root_UF_ANOVA.xlsx", row.names = FALSE)
