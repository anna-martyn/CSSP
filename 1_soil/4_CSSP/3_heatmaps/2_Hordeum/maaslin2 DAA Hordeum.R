...
pkg <- c("data.table", "magrittr", "ggplot2", "vegan", "Maaslin2", "patchwork",
         "RColorBrewer", "ComplexHeatmap", "colorRamp2", "ggh4x", "ggtext")
...
setnames(ASV_table, "#OTU ID", "ASV_ID")
ASV_table_full <- data.table(ASV_table)

meta_data <- fread(
  "../../../1_data/2_Barley/HordeumCSSP_AskovSoils_metadata.txt",
  drop = c(2,3,8)
)
meta_data_full <- data.table(meta_data)
taxonomy <- fread("../../../1_data/2_Barley/taxonomy_clean.tsv")
...
# Taxonomy annotation ----------------------------------------------------------
## Taxonomy ----
tax_bar <- taxonomy[ASV_ID %in% unique(htmp_hiabn$ASV_ID)]
# tax_bar <- tax_bar[match(unique(htmp_hiabn$ASV_ID), ASV_ID), c("ASV_ID", "Order")]
...
htmp_hiabn <- htmp_hiabn[,ASV_ID:=factor(ASV_ID, levels = tax_bar$ASV_ID)]
htmp_hiabn <- htmp_hiabn[order(ASV_ID)]
tax_bar$ASV_ID <- factor(tax_bar$ASV_ID, levels = tax_bar$ASV_ID)
p_tax <- ggplot(tax_bar, aes(x=ASV_ID, y=1, fill=Order)) +
...
heatmap <- ggplot(data = htmp_hiabn, 
                  mapping = aes(x = ASV_ID, y = y_axis, fill = DAA))+
  geom_tile()+
  ...
  scale_fill_manual(values = c("Depleted" = "darkblue",
                               "Enriched" = "#902121",
                               "NS" = "white"),
                    breaks = c("Depleted", "Enriched"))+
  labs(y = "Hordeum", fill = "DA ASV mutant *vs.* WT")+
  theme_bw()+
  theme(
    ...
    axis.text.y = element_markdown(size = 8, family = "Helvetica",
                                   colour = "black"),
    legend.text = element_text(size = 8, family = "Helvetica", colour = "black"),
    legend.title = element_markdown(size = 8, family = "Helvetica",
                                    colour = "black", face = "bold"),
    strip.text = element_text(size = 8, family = "Helvetica", colour = "black",
                              face = "bold"),
    legend.key.size = unit(0.25, 'cm'),
...
p_axis_title <- ggplot() + theme_void()

top_plot <- (p_tax + theme(legend.position = "none") |
               p_axis_title) + plot_layout(widths = c(0.9, 0.1))
bottom_plot <- (heatmap + theme(legend.position = "none") | bar_plot) + 
  plot_layout(widths = c(0.9, 0.1))
main_plot <- (top_plot/bottom_plot) + plot_layout(heights = c(0.05, 0.95))
main_plot

saveRDS(heatmap, "Heatmap_with_legend_Hv.rds")
saveRDS(bar_plot, "RA_Barplot_Hv.rds")
saveRDS(p_tax, "Taxonomy_with_legend_Hv.rds")
