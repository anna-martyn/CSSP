...
# Main figure ------------------------------------------------------------------
text <- ggplot() +
  annotate("text", x = 1, y = 1, label = "Hordeum", size = 8/.pt,
...
ggsave("CPCoA_PCoA_plot_Hv.pdf", CPCoA_PCoA_plot,
       width = 9, height = 5.5, units = "cm")
saveRDS(CPCoA_PCoA_plot, "CPCoA_PCoA_plot_Hv.rds")
saveRDS(CPCoA_plot_Root, "CPCoA_plot_with_legend_Hv.rds")
...
PCoA_plots <- annotate_figure(
  PCoA_plots, top = text_grob("Hordeum", size = 8, face = "bold")
)

saveRDS(PCoA_plots, "PCoA_plots_Hv.rds")
