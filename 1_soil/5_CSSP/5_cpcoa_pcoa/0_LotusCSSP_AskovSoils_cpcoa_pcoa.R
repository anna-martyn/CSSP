...
setnames(asv_table, "V1", "ASVid")

design <- fread("../../1_data/1_Lotus/LotusCSSP_AskovSoils_metadata.txt")
...
for(i in 1:length(cond)){
  ...
  asv_table_sub <- asv_table[,..sub_samples, with = FALSE]
  ...
  assign( paste("CPCoA_plot", cond[i], sep = "_"), CPCo_plot )
}
# PCoA plots ------------------------------------------------------------------
