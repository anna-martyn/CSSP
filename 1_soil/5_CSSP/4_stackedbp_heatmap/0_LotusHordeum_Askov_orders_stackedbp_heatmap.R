...
p1 <- ggplot(df.mean_order, aes(x=Genotype, y=RA, fill=Order)) +
  ...
  theme(
    axis.text.x = element_text(size=8, color="black", angle=50, vjust=1, hjust=1),
...
saveRDS(p1, file = "../6_final_figures/LotusHordeum_Askov_stackedbp_top20_meanRA.rds")
...
saveRDS(p_heatmap, file = "../6_final_figures/LotusHordeum_Askov_orders_heatmap.rds")
