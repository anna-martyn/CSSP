...
Lotus_df.long <- Lotus_df.long %>%
  mutate(Compartment = recode(Compartment, "Endosphere/Rhizoplane" = "Root"))
...
p1 <- ggplot(df.mean_order, aes(x=Soil, y=RA, fill=Order)) +
  ...
  theme(
    axis.text.x = element_text(size=20, color="black", angle=0, vjust=1),
    strip.text.x = element_text(size=20, face="bold")
  )

p1

ggsave("Soil_WT_stackedbp_meanRA.pdf", p1, width = 10, height = 6)
...
p_heatmap <- ggplot(df.plot, aes(x=Soil, y=Order, fill=RA)) +
  ...
  geom_text(aes(label=letter), na.rm=TRUE, size=6) +
  ...
  scale_y_discrete(
    limits = rev(levels(df.plot$Order))                  # move labels to right
  ) +
  guides(fill = guide_colorbar(
    title.position = "right",
    # title.hjust = 0.5,
    barwidth = 15,      # make the bar wider
...
ggsave("Soil_WT_heatmap_orders.pdf", p_heatmap, width = 12, height = 6)
