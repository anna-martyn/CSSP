...
alpha <- read.table("LotusCSSP_AskovSoils_chao1.txt", sep="\t", header=TRUE, row.names=1, check.names=FALSE)
design <- read.table("LotusCSSP_AskovSoils_metadata.txt", sep="\t", header=TRUE, row.names=1, check.names=FALSE)
...
# Save the plot.
ggsave("LotusCSSP_AskovSoils_WT_chao1_rfd.pdf", p1, width=7, height=5)
saveRDS(p1, file = "LotusCSSP_AskovSoils_WT_chao1_rfd.rds")
