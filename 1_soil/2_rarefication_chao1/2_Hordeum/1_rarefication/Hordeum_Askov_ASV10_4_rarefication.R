...
# Load the ASV table.
asv_table <- read.table("../../../1_data/2_Hordeum/HordeumCSSP_AskovSoils_ASVtable_10_4.tsv", sep="\t", header=TRUE, row.names=1, check.names=FALSE, comment.char = "", skip = 1)
...
# Check the second lowest depth, as the lowest one is only 86 reads.
...
# Based on this, we remove the low-depth sample by filtering for samples with >=1000 reads.
...
# Next, we rarefy the filtered ASV table using phyloseq.
