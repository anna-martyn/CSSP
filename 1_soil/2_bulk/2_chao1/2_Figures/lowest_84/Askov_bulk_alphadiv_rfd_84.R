...
p1 <- ggplot(index_bulk, aes(x=Soil, y=value, fill=Soil)) +
  ...
  geom_text(data=label_df, aes(x=Soil, y=y_position, label=Letters), inherit.aes=FALSE, size=8) +
  main_theme
...
# Save the plot.
ggsave("Askov_barley_bulk_chao1_rfd_84.pdf", p1, width=3, height=5)
saveRDS(p1, file = "Askov_barley_bulk_chao1_rfd_84.rds")

# Save ANOVA Tukey HSD output file.
write.csv(labels, file =  "Bulk_chao1_ANOVA_TukeyHSD_rfd_84.csv")
