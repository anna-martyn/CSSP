...
pkg <- c(
  "data.table", "ggplot2", "ggh4x", "gridExtra",
  "cowplot", "ggtext", "htmlTable", "Gmisc", "grid"
)
for(pk in pkg){
...
# Defining colours for genotypes, soils, and orders
colors_genotype <- c(
  "WT" = "#A9C289",
  "symrk" = "#FEDA8B",
  "ccamk" = "#FDB366",
...
connect1 <- connectGrob(start = train, end = mod1, "N", "l", arrow_obj = custom_arrow)
connect2 <- connectGrob(start = train, end = mod2, "N", "l", arrow_obj = custom_arrow)
connect3 <- connectGrob(start = mod1, end = pred, "vertical", arrow_obj = custom_arrow)
...
new_connection <- linesGrob(x = x_coords, y = y_coords, gp = gpar(fill = "black"))

connect5 <-  connectGrob(start = pred, end = diamond_box, "vertical", arrow_obj = custom_arrow)
connect6 <-  connectGrob(start = mod2, end = pred2, "vertical", arrow_obj = custom_arrow)
...
connect9 <-  connectGrob(start = pred2, end = concl, "vertical", arrow_obj = custom_arrow)
...
ggplot(data = res) +
  ...
  theme(
    ...
    axis.text.x = element_text(size = 6, family = "Helvetica", colour = "black"),
    legend.text = element_markdown(size = 6, family = "Helvetica", colour = "black"),
    legend.title = element_text(size = 6, family = "Helvetica", colour = "black"),
...
tax_summary[!(Order %in% colors_order$Order), Order:= "Other"]
tax_summary[,":="(
...
#  Combining plots ----
tg <- tableGrob(ratio_amount, theme = ttheme_default(base_size = 6), rows = NULL)

gg1 <- plot_grid(
  grob, tg, labels = c("A", "C"), rel_heights = c(0.8, 0.2), ncol = 1, label_size = 12
)

gg2 <- plot_grid(
  g1, g2, labels = c("B", "D"), rel_heights = c(0.5, 0.5), ncol = 1, label_size = 12
)

gg <- plot_grid(gg1, gg2, ncol = 2, label_size = 12)
