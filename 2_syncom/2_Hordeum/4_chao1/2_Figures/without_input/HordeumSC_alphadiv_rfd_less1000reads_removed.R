...
alpha <- read.table("../../1_Scripts/without_input/allASVs/HordeumSC_allASVs_chao1.txt", sep="\t", header=TRUE, row.names=1, check.names=FALSE)
alpha_filt <- read.table("../../1_Scripts/without_input/matchedASVsonly/HordeumSC_matchedASVsonly_chao1.txt", sep="\t", header=TRUE, row.names=1, check.names=FALSE)
design <- read.table("HordeumSC_metadata.txt", sep="\t", header=TRUE, row.names=1, check.names=FALSE)

# For both dataframes:
# 1. add metadata info
# 2. set genotype and compartment factor levels
...
run_analysis_plot(index,
                  colors,
                  title = "Chao1 index",
                  output_pdf = "HordeumSC_chao1_allASVs.pdf",
                  output_rds = "HordeumSC_chao1_allASVs.rds")

run_analysis_plot(index_filt,
                  colors,
                  title = "Chao1 index",
                  output_pdf = "HordeumSC_chao1_filteredASVs.pdf",
                  output_rds = "HordeumSC_chao1_filteredASVs.rds")
